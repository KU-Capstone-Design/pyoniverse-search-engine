...
테스트 내용: 100개의 무작위 데이터로 연산을 10회 실시해 평균 속도를 구한다.
테스트 실행(line_profiler 사용): kernprof -l -v {script}
테스트 결과: Space.__call__ 자체가 시간이 오래걸린다(30ms/name) -> 10,000개의 데이터로 확장하면 300sec ~ 5분
결론: Spacing 결과를 저장하자
"""
import json
