...
class SearchAI:
    ...
    def __init__(
        self, model_type: Literal["lexical", "sentence"], model_name: str, model: Any, corpus: Union[List[dict], str]
    ):
        """
        :param model_type: lexical or sentence model(검색 방식이 달라진다)
        :param model: EmbeddingAI로 생성된 모델
        :param model_name: 모델 이름
        :param corpus: [{"embedding": ..., "id": ..., "name": ...}] 형식의 검색 대상 혹은 해당 코퍼스 경로
        """
        ...
        if isinstance(corpus, str):
            # load corpus
            self.__corpus = self.__load_corpus(corpus)
        else:
